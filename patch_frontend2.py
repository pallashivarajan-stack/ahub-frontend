...
states_code = '''
  ...
  const handleFormSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setInternships((prev) =>
        prev.map((item) => {
          if (item.domain === values.domain && item.status !== "Applied") {
...
if 'const [loadingConfig' not in content:
    # We will slice out the old state and handleFormSubmit
    start_str = '  const [formData, setFormData] = useState({'
    end_str = '    );\n  };\n'
    ...
    if idx1 != -1 and idx2 != -1:
        ...
        content = content[:idx1] + states_code + content[end_idx:]

    # Fix handleApplyClick to use updateValue instead of setFormData
    content = content.replace(
        'setFormData((prev) => ({\n      ...prev,\n      domain: internship.domain,\n    }));',
        'updateValue("domain", internship.domain);'
    )
...
start_str = '{/* REGISTRATION FORM SECTION */}'
end_str = '{/* BENEFITS STRIP */}'

idx1 = content.find(start_str)
idx2 = content.find(end_str, idx1)

if idx1 != -1 and idx2 != -1:
    content = content[:idx1] + new_form_section + '\n        ' + content[idx2:]
