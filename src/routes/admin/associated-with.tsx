...
import { isLoggedIn, loadAssociatedWithData, saveAssociatedWithData, getAssetOptions } from "@/services/adminService";
import { AdminSidebar } from "@/components/admin/AdminSidebar";
import { Plus, Trash2, Save, Check, X } from "lucide-react";

export const Route = createFileRoute("/admin/associated-with")({
  component: AdminAssociatedWithPage,
});

type AssetOption = { path: string; url: string; label: string };

function AdminAssociatedWithPage() {
  const navigate = useNavigate();
  const [logos, setLogos] = useState<string[]>(() => loadAssociatedWithData());
  ...
  const handleSave = async () => {
    setSaving(true);
    await saveAssociatedWithData(logos);
    setSaving(false);
    setSaved(true);
    ...
    <div className="flex min-h-screen bg-slate-50">
      ...
      <main className="flex-1 overflow-auto">
        <div className="border-b border-slate-200 bg-white px-8 py-4 flex items-center justify-between">
          <div>
            <h1 className="text-xl font-bold text-slate-900">Associated With Logos</h1>
            <p className="text-xs text-slate-500 mt-0.5">Controls logos for the "Associated With" section</p>
          </div>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40" onClick={() => setShowAdd(false)}>
          <div className="w-full max-w-lg rounded-xl bg-white p-6 shadow-xl border border-slate-200 mx-4" onClick={(e) => e.stopPropagation()}>
            <div className="flex items-center justify-between mb-4">
              <h3 className="text-lg font-semibold text-slate-900">Add Associated With Logo</h3>
