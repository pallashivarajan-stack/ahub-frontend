...
import { isLoggedIn, loadPartnersLogosData, savePartnersLogosData, getAssetOptions } from "@/services/adminService";
import { AdminSidebar } from "@/components/admin/AdminSidebar";
import { Plus, Trash2, Save, Check, X } from "lucide-react";

export const Route = createFileRoute("/admin/partners-logos")({
  component: AdminPartnersLogosPage,
});

type AssetOption = { path: string; url: string; label: string };

function AdminPartnersLogosPage() {
  const navigate = useNavigate();
  const [logos, setLogos] = useState<string[]>(() => loadPartnersLogosData());
  ...
  const handleSave = async () => {
    setSaving(true);
    await savePartnersLogosData(logos);
    setSaving(false);
    setSaved(true);
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40" onClick={() => setShowAdd(false)}>
          <div className="w-full max-w-lg rounded-xl bg-white p-6 shadow-xl border border-slate-200 mx-4" onClick={(e) => e.stopPropagation()}>
            <div className="flex items-center justify-between mb-4">
              <h3 className="text-lg font-semibold text-slate-900">Add Partner Logo</h3>
